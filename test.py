...
def next(state, action):
    ...
    return is_valid, state, valid_action, has_weapon, in_gas, has_attack
...
def get_reward(state, valid, action, has_weapon, in_gas, has_attack):
    map_ = state[0, :, :, 0]
    gas_pos = state[1, :, :, 0]
    player_pos = np.argwhere(state[0, :, :, 1] == 1)[0]
    enemy_pos = np.argwhere(state[0, :, :, 2] == 1)[0]
    ...
    for _ in range(1000000):
        ...
        while 1:
            ...
            while True:
                ...
                valid, state, actions, has_weapon, in_gas, has_attack = next(state, action)  # 状态转移
                observation_ = np.hstack((state.flatten(), actions))
                done = 0 if action != 8 and valid else 1  # 本轮是否终止
                reward = get_reward(state, valid, action, has_weapon, in_gas, has_attack)
